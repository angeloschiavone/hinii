 ...
 * </ul>
 * 
 * Create an instance of this class, then call one of the three getBoundary()
 * methods and compare against a Date object to see if it falls before or after.
 * 
 * @date March 22, 2010
...
public class CheckinTimestampSort {
	...
	private static Date[] getDateObjects() {
		...
		if (DEBUG)
			Log.d(TAG, "Now: " + cal.getTime().toGMTString());

		// Three hours ago or newer.
		cal.add(Calendar.HOUR, -3);
		Date dateRecent = cal.getTime();
		if (DEBUG)
			Log.d(TAG, "Recent: " + cal.getTime().toGMTString());
		...
		if (DEBUG)
			Log.d(TAG, "Today: " + cal.getTime().toGMTString());

		// Yesterday.
		cal.add(Calendar.DAY_OF_MONTH, -1);
		Date dateYesterday = cal.getTime();
		if (DEBUG)
			Log.d(TAG, "Yesterday: " + cal.getTime().toGMTString());
