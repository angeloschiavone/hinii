...
public class FetchImageForViewIntent extends Activity {
	...
	public static final String IMAGE_URL = Hiniid.PACKAGE_NAME
			+ ".FetchImageForViewIntent.IMAGE_URL";
	public static final String CONNECTION_TIMEOUT_IN_SECONDS = Hiniid.PACKAGE_NAME
			+ ".FetchImageForViewIntent.CONNECTION_TIMEOUT_IN_SECONDS";
	public static final String READ_TIMEOUT_IN_SECONDS = Hiniid.PACKAGE_NAME
			+ ".FetchImageForViewIntent.READ_TIMEOUT_IN_SECONDS";
	public static final String PROGRESS_BAR_TITLE = Hiniid.PACKAGE_NAME
			+ ".FetchImageForViewIntent.PROGRESS_BAR_TITLE";
	public static final String PROGRESS_BAR_MESSAGE = Hiniid.PACKAGE_NAME
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (retained != null && retained instanceof StateHolder) {
			...
			if (posdot < 0) {
				Log.e(TAG,
						"FetchImageForViewIntent requires a url to an image resource with a file extension in its name.");
				finish();
				return;
			}

			String progressBarTitle = getIntent().getStringExtra(
					PROGRESS_BAR_TITLE);
			if (progressBarTitle == null) {
				progressBarTitle = "Fetching Image";
			}
			String progressBarMessage = getIntent().getStringExtra(
			...
			mStateHolder.startTask(FetchImageForViewIntent.this, url,
					url.substring(posdot), progressBarTitle,
					progressBarMessage,
					getIntent().getIntExtra(CONNECTION_TIMEOUT_IN_SECONDS, 30),
					getIntent().getIntExtra(READ_TIMEOUT_IN_SECONDS, 30));
		}
	}
	...
	public void onResume() {
		...
		if (mStateHolder.getIsRunning()) {
			startProgressBar(mStateHolder.getProgressTitle(),
					mStateHolder.getProgressMessage());
		}
	}
	...
	private void onFetchImageTaskComplete(Boolean result, String path,
			String extension, Exception ex) {
		try {
			...
			if (result.equals(Boolean.TRUE)) {
				...
				NotificationsUtil.ToastReasonForFailure(
						FetchImageForViewIntent.this, ex);
			}
		} finally {
	...
	private boolean launchViewIntent(String outputPath, String extension) {
		...
		try {
			...
			Log.e(TAG, "Error opening downloaded image from temp location: ",
					ex);
			Toast.makeText(this,
					"No application could be found to diplay the full image.",
					Toast.LENGTH_SHORT);
			return false;
		}
		...
		try {
			...
			Toast.makeText(this, "There was an error displaying the image.",
					Toast.LENGTH_SHORT);
			return false;
		}
	...
	private static class FetchImageTask extends AsyncTask<Void, Void, Boolean> {
		...
		public FetchImageTask(FetchImageForViewIntent activity, String url,
				String extension, int connectionTimeoutInSeconds,
				int readTimeoutInSeconds) {
			mActivity = activity;
			mUrl = url;
			mExtension = extension;
			mOutputPath = Environment.getExternalStorageDirectory() + "/"
					+ TEMP_FILE_NAME;
		...
		protected Boolean doInBackground(Void... params) {
			try {
				saveImage(mUrl, mOutputPath, mConnectionTimeoutInSeconds,
				...
				if (DEBUG)
					Log.d(TAG,
							"FetchImageTask: Exception while fetching image.",
							e);
				mReason = e;
			}
		...
		protected void onPostExecute(Boolean result) {
			if (DEBUG)
				Log.d(TAG, "FetchImageTask: onPostExecute()");
			if (mActivity != null) {
				mActivity.onFetchImageTaskComplete(result, mOutputPath,
						mExtension, mReason);
			}
		}
		...
		protected void onCancelled() {
			if (mActivity != null) {
				mActivity.onFetchImageTaskComplete(null, null, null,
						new Exception("Fetch image from url cancelled."));
			}
		}
	}

	public static void saveImage(String urlImage, String savePath,
		...
		if (offset != contentLength) {
			Log.e(TAG, "Error fetching image, only read " + offset
					+ " bytes of " + contentLength + " total.");
			throw new HiniiException(
					"Error fetching full image, please try again.");
		}

		// This will fail if the user has no sdcard present, catch it
		// specifically
		// to alert user.
		try {
			...
			throw new HiniiException(
					"Error opening fetched image, make sure an sdcard is present.");
		}
	}
	...
	private static class StateHolder {
		...
		public void startTask(FetchImageForViewIntent activity, String url,
				String extension, String progressBarTitle,
				String progressBarMessage, int connectionTimeoutInSeconds,
				int readTimeoutInSeconds) {
			mIsRunning = true;
